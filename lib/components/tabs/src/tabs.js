...
var pane = require('./utils/pane.js');
...
var Tabs = vue.defineComponent({
  ...
  setup(props, {
    ...
    const nav$ = vue.ref();
    const panes = vue.shallowReactive({});
    const orderedPanes = vue.shallowRef([]);
    ...
    {
      const registerPane = (pane$1) => {
        panes[pane$1.uid] = pane$1;
        orderedPanes.value = pane.getOrderedPanes(vm, panes);
      };
